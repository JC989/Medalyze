        ...
        if st.button("Upload & Process All Transcripts"):
            ...
            for doc in upload_transcripts:
                with st.spinner(f"Uploading {doc.name}..."):
                    ...
                    try:
                        response = requests.post(API_UPLOAD_DOCUMENT, files=files)
...
with tab2:
    ...
    for result in st.session_state["all_analysis_results"]:
        ...
        # Row labels: either transcript-specific or default numbering
    ...
    if st.button("Send Heatmap to NeuralSeek for Email"):
        ...
        try:
            upload_resp = requests.post(API_UPLOAD_HEATMAP, json=payload)
...
# Placeholder API URLs
# ==============================
API_UPLOAD_DOCUMENT = "https://fake-neuralseek-server.com/api/v1/upload_document"
API_ANALYSIS_STATUS = "https://fake-neuralseek-server.com/api/v1/get_analysis"
API_UPLOAD_HEATMAP = "https://fake-neuralseek-server.com/api/v1/upload_heatmap"
